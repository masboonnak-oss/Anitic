...
if (overlaySock && OVERLAY_USER) {
  overlaySock.on('connect', () => overlaySock.emit('joinRoom', { username: OVERLAY_USER }));
}
...
function OverlayView() {
  ...
  useEffect(() => {
    ...
    return () => { overlaySock.off('tiktokMember', onMember); overlaySock.off('tiktokGift', onGift); };
  }, []);
  ...
  return (
    <>
      ...
      {isPreview && (
        <div style={{ position:'fixed', top:16, left:16, right:16, display:'flex', justifyContent:'space-between', zIndex:50 }}>
          <div style={{ background:'rgba(10,0,20,.85)', border:'1px solid rgba(155,81,224,.35)', borderRadius:10, padding:'7px 14px', backdropFilter:'blur(12px)' }}>
            <span style={{ fontSize:11, fontWeight:700, color:'rgba(155,81,224,.9)', letterSpacing:'.15em' }}>👁 PREVIEW MODE</span>
            {OVERLAY_USER && <span style={{ fontSize:11, color:'rgba(255,255,255,.35)', marginLeft:8 }}>@{OVERLAY_USER}</span>}
          </div>
          <div style={{ background:'rgba(10,0,20,.85)', border:'1px solid rgba(155,81,224,.2)', borderRadius:9, padding:'6px 12px', fontSize:11, color:'rgba(255,255,255,.35)' }}>
            OBS: ใช้ URL โดยไม่ต้องมี ?preview
          </div>
        </div>
      )}
      ...
      {isPreview && queue.length === 0 && (
        <div style={{ position:'fixed', bottom:32, left:32, background:'rgba(10,0,20,.75)', border:'1px dashed rgba(155,81,224,.3)', borderRadius:12, padding:'18px 24px', color:'rgba(155,81,224,.5)', fontSize:13, fontWeight:600, backdropFilter:'blur(12px)' }}>
          🎭 รอคนเข้าห้อง... (preview mode)
        </div>
      )}
    </>
  );
