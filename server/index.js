...
/* ══════════════════════════════════════════════════════════
   TIKFINITY — Top Gifter & VIP Entrance Overlay
   ══════════════════════════════════════════════════════════
   ตั้งค่า Token:
   1. เปิด Tikfinity App → Settings → API / WebSocket Token
   2. Copy token ที่แสดง
   3. Replit → Tools → Secrets → "+ New secret"
      Key: TIKFINITY_TOKEN   Value: (token ของคุณ)
   ════════════════════════════════════════════════════════ */

const GIFTERS_FILE = path.join(CACHE_DIR, '_gifters.json');

function loadGifters() {
  try { return JSON.parse(fs.readFileSync(GIFTERS_FILE, 'utf8')); }
  catch (_) { return {}; }
}
function saveGifters(data) {
...
app.get('/api/overlay/gifters', (req, res) => {
  ...
  const sorted = Object.values(gifters).sort((a, b) => b.diamonds - a.diamonds);
  res.json(sorted);
});

/* DELETE — reset gifter scores */
app.delete('/api/overlay/gifters', authMiddleware, superAdminMiddleware, (req, res) => {
  saveGifters({});
  console.log('[overlay] gifter scores reset');
  res.json({ ok: true });
});

/* ── Tikfinity WebSocket connection ── */
let tikfinityWs     = null;
let reconnectTimer  = null;
const TIKFINITY_TOKEN = process.env.TIKFINITY_TOKEN || '';

function connectTikfinity() {
  if (!TIKFINITY_TOKEN) {
    console.log('[tikfinity] ⚠️  TIKFINITY_TOKEN not set — overlay disabled. Add it in Secrets.');
    return;
  }

  const url = `ws://localhost:18331/?token=${TIKFINITY_TOKEN}`;
  console.log('[tikfinity] connecting to Tikfinity WebSocket...');
  ...
  tikfinityWs.on('message', (raw) => {
    let msg;
    try { msg = JSON.parse(raw); } catch (_) { return; }

    const event = msg.event || msg.type;

    /* ── Gift event → accumulate diamonds ── */
    if (event === 'gift') {
      const { uniqueId, nickname, profilePicture, diamondCount, amount, level } = msg.data || msg;
      if (!uniqueId) return;
      const diamonds = (diamondCount || 0) * (amount || 1);
      const gifters  = loadGifters();
      if (!gifters[uniqueId]) {
        gifters[uniqueId] = { uniqueId, nickname, profilePicture, level: level || 0, diamonds: 0 };
      }
      gifters[uniqueId].diamonds   += diamonds;
      gifters[uniqueId].nickname    = nickname    || gifters[uniqueId].nickname;
      gifters[uniqueId].profilePicture = profilePicture || gifters[uniqueId].profilePicture;
      gifters[uniqueId].level       = level       || gifters[uniqueId].level;
      saveGifters(gifters);
      console.log(`[tikfinity] 💎 gift: ${nickname} +${diamonds} (total: ${gifters[uniqueId].diamonds})`);
    }

    /* ── Join event → check top gifter or level 20+ ── */
    if (event === 'join' || event === 'memberJoin') {
      const { uniqueId, nickname, profilePicture, level } = msg.data || msg;
      if (!uniqueId) return;

      const gifters = loadGifters();
      const sorted  = Object.values(gifters).sort((a, b) => b.diamonds - a.diamonds);
      const isTopGifter = sorted.length > 0 && sorted[0].uniqueId === uniqueId;
      const isHighLevel  = (level || 0) >= 20;

      if (!isTopGifter && !isHighLevel) return;
      ...
      const payload = {
        ...
        level: level || 0,
        isTopGifter,
        diamonds: gifters[uniqueId]?.diamonds || 0,
      };

      console.log(`[tikfinity] 🚪 entrance: ${nickname} (top=${isTopGifter}, lv=${level})`);
      io.emit('overlay:entrance', payload);
    }
  });

  tikfinityWs.on('error', (err) => {
    console.error('[tikfinity] error:', err.message);
  });

  tikfinityWs.on('close', () => {
    console.log('[tikfinity] disconnected — reconnecting in 5s...');
    tikfinityWs = null;
    if (!reconnectTimer) {
...
// Start Tikfinity connection on server boot
connectTikfinity();
